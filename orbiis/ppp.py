...
def nut_iau1980(t_, f):
    ...
    dpsi *= sn.rCST.AS2R*1e-4
    deps *= sn.rCST.AS2R*1e-4
    return dpsi, deps
...
def ast_args(t_):
    ...
    for k_ in range(5):
        ...
        f[k_] = np.fmod(f[k_]*sn.rCST.AS2R, 2.0*np.pi)
    return f
...
def utc2gmst(t_, ut1_utc=0):
    """ UTC to GMST """
    ep0 = sn.epoch2time([2000, 1, 1, 12, 0, 0])
    tut = sn.timeadd(t_, ut1_utc)
    ep = sn.time2epoch(tut)
    tut0 = sn.epoch2time([ep[0], ep[1], ep[2], 0, 0, 0])
    ut = ep[3]*3600+ep[4]*60+ep[5]
    t1 = sn.timediff(tut0, ep0)/sn.rCST.CENTURY_SEC
    t2 = t1**2
    t3 = t2*t1
    gmst0 = 24110.54841+8640184.812866*t1+0.093104*t2-6.2e-6*t3
    gmst = gmst0+1.002737909350795*ut
    return np.fmod(gmst, sn.rCST.DAY_SEC)*(2.0*np.pi/sn.rCST.DAY_SEC)


def eci2ecef(tgps, erpv):
    """ ECI to ECEF conversion matrix """
    tutc = sn.gpst2utc(tgps)
    ep0 = sn.epoch2time([2000, 1, 1, 12, 0, 0])
    dt = sn.timediff(tgps, ep0)
    t_ = (dt+19+32.184)/sn.rCST.CENTURY_SEC
    t2 = t_**2
    t3 = t2*t_
    f = ast_args(t_)
    # iau1976 precession
    ze = (2306.2181*t_+0.30188*t2+0.017998*t3)*sn.rCST.AS2R
    th = (2004.3109*t_-0.42665*t2-0.041833*t3)*sn.rCST.AS2R
    z = (2306.2181*t_+1.09468*t2+0.018203*t3)*sn.rCST.AS2R
    eps = (84381.448-46.8150*t_-0.00059*t2+0.001813*t3)*sn.rCST.AS2R
    P = Rz(-z)@Ry(th)@Rz(-ze)
    ...
    gast += (0.00264*sin(f[4])+0.000063*sin(2.0*f[4]))*sn.rCST.AS2R
...
def sunmoonpos(tutc, erpv=np.zeros(5)):
    ...
    tut = sn.timeadd(tutc, erpv[2])
    ep0 = sn.epoch2time([2000, 1, 1, 12, 0, 0])
    t_ = sn.timediff(tut, ep0)/sn.rCST.CENTURY_SEC
    f = ast_args(t_)
    ...
    rs = sn.rCST.AU*(1.000140612-0.016708617*cos(Ms)-0.000139589*cos(2.0*Ms))
    c_l = cos(ls)
    s_l = sin(ls)
    ...
    rm = sn.rCST.RE_WGS84/sin(np.deg2rad(u))
...
def shapiro(rsat, rrcv):
    ...
    corr = (2*sn.rCST.GME/sn.rCST.CLIGHT**2)*np.log((rs+rr+rrs)/(rs+rr-rrs))
    return corr


def windupcorr(time, rs, vs, rr, phw):
    """ calculate windup correction """
    we = np.array([0, 0, sn.rCST.OMGE])
    # rsun, rmoon, gmst = sunmoonpos(gn.gpst2utc(time))
    ek = sn.vnorm(rr-rs)
    ezs = sn.vnorm(-rs)
    ess = sn.vnorm(vs+np.cross(we, rs))
    eys = sn.vnorm(np.cross(ezs, ess))
...
def tide_pl(eu, rp, GMp, pos):
    ...
    r = np.linalg.norm(rp)
    ep = rp/r
    K2 = GMp/sn.rCST.GME*sn.rCST.RE_WGS84**4/r**3
    K3 = K2*sn.rCST.RE_WGS84/r
    latp = asin(ep[2])
...
def solid_tide(rsun, rmoon, pos, E, gmst, flag=True):
    ...
    # time domain
    eu = E[2, :]
    dr1 = tide_pl(eu, rsun, sn.rCST.GMS, pos)
    dr2 = tide_pl(eu, rmoon, sn.rCST.GMM, pos)
    # frequency domain
    s_2l = sin(2.0*pos[0])
...
if __name__ == '__main__':
    ...
    tgps_ = sn.epoch2time([2021, 3, 19, 0, 0, 0])
    ...
    if flg_tide:
        ...
        dr_ = np.zeros((n, 3))
        for k in range(n):
            tn = sn.timeadd(tgps_, k*300)
            t[k] = sn.timediff(tn, tgps_)
            dn_ = tidedisp(sn.gpst2utc(tn), pos_, erpv_)
            dr_[k, :] = sn.ecef2enu(pos_, dn_)
        ...
        if flg_pwup:
            bdir = '../data/'
            navfile = os.path.join(bdir, '30340780.21q')
            
            if not os.path.isfile(navfile):
                raise FileNotFoundError(f"The file {navfile} does not exist.")
            
            nav = sn.Nav()
            dec = rnxdec()
            nav = dec.decode_nav(navfile, nav)
            rr_ = sn.pos2ecef(pos_)
            sat = sn.prn2sat(sn.uGNSS.QZS, 194)
