...
def rtkinit(nav, pos0=np.zeros(3)):
    ...
    nav.ratio = 0
    nav.thresar = [2]
    nav.nx = nav.na+sn.uGNSS.MAXSAT*nav.nf
    ...
    # obs index
    i0 = {sn.uGNSS.GPS: 0, sn.uGNSS.GAL: 0, sn.uGNSS.QZS: 0}
    i1 = {sn.uGNSS.GPS: 1, sn.uGNSS.GAL: 2, sn.uGNSS.QZS: 1}
    freq0 = {sn.uGNSS.GPS: nav.freq[0], sn.uGNSS.GAL: nav.freq[0],
             sn.uGNSS.QZS: nav.freq[0]}
    freq1 = {sn.uGNSS.GPS: nav.freq[1], sn.uGNSS.GAL: nav.freq[2],
             sn.uGNSS.QZS: nav.freq[1]}
    nav.obs_idx = [i0, i1]
...
def zdres(nav, obs, rs, dts, svh, rr, rtype=1):
    ...
    if nav.tidecorr:
        pos = sn.ecef2pos(rr_)
        disp = tidedisp(sn.gpst2utc(obs.t), pos)
        rr_ += disp
    pos = sn.ecef2pos(rr_)
    for i in range(n):
        sys, _ = sn.sat2prn(obs.sat[i])
        if svh[i] > 0 or sys not in nav.gnss_t or obs.sat[i] in nav.excl_sat:
            continue
        r, e[i, :] = sn.geodist(rs[i, :], rr_)
        _, el[i] = sn.satazel(pos, e[i, :])
        if el[i] < nav.elmin:
            continue
        r += -_c*dts[i]
        zhd, _, _ = sn.tropmodel(obs.t, pos, np.deg2rad(90.0), 0.0)
        mapfh, _ = sn.tropmapf(obs.t, pos, el[i])
        r += mapfh*zhd

        dant = sn.antmodel(nav, el[i], nav.nf, rtype)
...
def ddidx(nav, sat):
    ...
    for m in range(sn.uGNSS.GNSSMAX):
        k = na
        for f in range(nav.nf):
            for i in range(k, k+n):
                sat_i = i-k+1
                sys, _ = sn.sat2prn(sat_i)
...
def restamb(nav, bias, nb):
    ...
    for m in range(sn.uGNSS.GNSSMAX):
        for f in range(nav.nf):
            n = 0
            index = []
            for i in range(sn.uGNSS.MAXSAT):
                sys, _ = sn.sat2prn(i+1)
...
def udstate(nav, obs, obsb, iu, ir):
    ...
    tt = sn.timediff(obs.t, nav.t)
    ns = len(iu)
    sys = []
    sat = obs.sat[iu]
    for sat_i in obs.sat[iu]:
        sys_i, _ = sn.sat2prn(sat_i)
        sys.append(sys_i)
    ...
    for f in range(nav.nf):
        ...
        for i in range(ns):
            ...
            bias[i] = cp-pr*freq/sn.rCST.CLIGHT
